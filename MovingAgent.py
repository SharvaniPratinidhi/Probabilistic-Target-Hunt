...
class Agent:
    ...
    def searchTarget(self, currentxi, currentyi):
        maxprob = 0
        maxprobcells = []
        for i in range(self.dimension):
            for j in range(self.dimension):
                distance = abs(i - currentxi) + abs(j - currentyi)
                p = self.belief[i][j] / (distance + 1)
                if (p > maxprob):
                    maxprob = p
                    maxprobcells = [(i, j)]
                elif (p == maxprob):
    ...
    def solve(self):
        ...
        while self.target == NOT_FOUND:
            ...
            result, row, col = self.searchTarget(currxi, curryi)
            currxi = row
            curryi = col
            if result == True:
                self.target = FOUND
